...
// Removed the setTimeout to avoid competing initializations
...
class SessionManager {
  constructor() {
    ...
    // Trigger initialization of the SDK immediately (no timeout)
    _CastContext.default.initialize().then(() => {
      console.log('[GoogleCast] Context initialized, checking session');
      return this.getCurrentCastSession();
    }).catch(err => {
  ...
  async endCurrentSession(stopCasting = false) {
    try {
      console.log('[GoogleCast] Ending current session, stopCasting:', stopCasting);
  ...
  async getCurrentCastSession() {
    try {
      ...
      if (session) {
        ...
        return new _CastSession.default(session);
      } else {
  ...
  onSessionStarting(handler) {
    ...
    return this.eventEmitter.addListener(Native.SESSION_STARTING, ({
      ...
      console.log('[GoogleCast] Session starting with ID:', session === null || session === void 0 ? void 0 : session.id);
  ...
  onSessionStarted(handler) {
    ...
    return this.eventEmitter.addListener(Native.SESSION_STARTED, ({
      ...
      console.log('[GoogleCast] Session started with ID:', session === null || session === void 0 ? void 0 : session.id);
      handler(new _CastSession.default(session));
    });
  }
